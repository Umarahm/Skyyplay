...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    if (id && type) {
      ...
      // Set season and episode if provided
      if (season && type === "tv") {
        setSelectedSeason(Number.parseInt(season))
        setDisplaySeason(Number.parseInt(season))
      }
      if (episode && type === "tv") {
        setSelectedEpisode(Number.parseInt(episode))
      }
      ...
      fetchContent(Number.parseInt(id), type as "movie" | "tv", season ? Number.parseInt(season) : 1)
    }
  }, [searchParams])
  ...
  const fetchContent = async (id: number, type: "movie" | "tv", seasonNumber: number) => {
    setIsLoading(true)
    try {
      ...
      if (type === "tv" && "number_of_seasons" in data) {
        ...
        await updateEpisodes(id, seasonNumber)
      }
  ...
  const updateEpisodes = async (tvId: number, seasonNumber: number) => {
    ...
    try {
      ...
      setSeasonData(data)
      // Reset to episode 1 when switching seasons, unless a specific episode was requested via URL
  ...
  const loadEpisode = (episodeNumber: number) => {
    setSelectedSeason(displaySeason)
                    ...
                    <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-hide">
                      {seasons.map((season) => (
                        <button
                          ...
                          className={`flex-shrink-0 px-4 py-2 rounded-lg transition-all duration-300 whitespace-nowrap ${displaySeason === season ? "btn-primary" : "btn-secondary"
                            }`}
                        >
                          S{season}
                        </button>
